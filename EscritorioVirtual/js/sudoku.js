...
class Sudoku{
    ...
    createStructure(){
        ...
        var sections = new Array(this.maxFilas);
        for(let i = 0; i < this.maxFilas; i++){
            sections[i] = document.createElement("section");
            sections[i].setAttribute("data-name", "game");
        }
        var p = null;
        for(let i = 0; i < this.maxFilas; i++)
            for(let j = 0; j < this.maxCols; j++){
                ...
                var subGridRowStart = (i - i%3);
                var subGridColStart = (j - j%3);
                var subGrid =  subGridRowStart + Math.floor(subGridColStart/3);
                sections[subGrid].appendChild(p);
            }
    ...
    getCellsOrderedByRow(){
        var cells = document.querySelectorAll("p");
        var retCells = new Array(this.maxFilas);
        for(let i = 0; i < this.maxCols; i++)
            retCells[i] = new Array(this.maxCols);
        var iModifier = 0;
        for(let i = 0; i < this.maxFilas; i++){
            if(i%3 === 0 && i !== 0)
                iModifier += 18;
            var iModified = (i * 3) + iModifier;
            var jModifier = 0;
            for(let j = 0; j < this.maxCols; j++){
                if(j%3 === 0 && j !== 0)
                    jModifier += 6;
                var jModified = j + jModifier;
                retCells[i][j] = cells[(iModified) + jModified];
                }
        }
